...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eac="urn:isbn:1-931666-33-4"
    ...
    <xsl:template name="tFurther">
        ...
        <xsl:choose>
            <xsl:when
                ...
                <xsl:for-each
                    ...
                    <xsl:for-each select="eac:relationEntry[1]">
                        ...
                        <xsl:choose>
                            <xsl:when
                                test="contains(../@xlink:href,'oclc/') and following-sibling::eac:relationEntry[@localType='isbn']">
                                <xsl:value-of select="normalize-space(.)"/>
                                <xsl:text>''. </xsl:text>
                            ...
                            <xsl:when
                                test="contains(../@xlink:href,'oclc/') and not(following-sibling::eac:relationEntry[@localType='isbn'])">
                                <xsl:value-of select="normalize-space(.)"/>
                                <xsl:text>''. </xsl:text>
                                <xsl:text>{{OCLC|</xsl:text>
                                <xsl:value-of select="substring-after(../@xlink:href,'oclc/')"/>
                            ...
                            <xsl:when
                                test="following-sibling::eac:relationEntry[@localType='isbn'] and not(contains(../@xlink:href,'oclc/'))">
                                <xsl:value-of select="normalize-space(.)"/>
                                <xsl:text>''. </xsl:text>
                                <xsl:text>ISBN </xsl:text>
                                <xsl:value-of
            ...
            <xsl:when
                test="eac:eac-cpf/eac:cpfDescription/eac:relations/eac:resourceRelation[not(@resourceRelationType) and not(@xlink:role='resource')]">
                <xsl:for-each
    ...
    <xsl:template name="tPub">
        ...
        <xsl:choose>
            <xsl:when
                test="eac:eac-cpf/eac:cpfDescription/eac:relations/eac:resourceRelation[@resourceRelationType='creatorOf' and @xlink:role='resource']">
                ...
                <xsl:for-each
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="normalize-space(child::eac:relationEntry[@localType='creator'])">
                            <xsl:text>''</xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>''</xsl:text>
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="contains(@xlink:href,'oclc/') and eac:relationEntry[@localType='isbn']">
                            <xsl:text>{{OCLC|</xsl:text>
                            <xsl:value-of select="substring-after(@xlink:href,'oclc/')"/>
                            <xsl:text>}}, </xsl:text>
                            <xsl:text>ISBN </xsl:text>
                        ...
                        <xsl:when
                            test="contains(@xlink:href,'oclc/') and not(eac:relationEntry[@localType='isbn'])">
                            <xsl:text>{{OCLC|</xsl:text>
                            <xsl:value-of select="substring-after(@xlink:href,'oclc/')"/>
                            <xsl:text>}}</xsl:text>
                        </xsl:when>
                        <xsl:when
                            test="eac:relationEntry[@localType='isbn'] and not(contains(@xlink:href,'oclc/'))">
                            <xsl:text>ISBN </xsl:text>
                            <xsl:value-of select="eac:relationEntry[@localType='isbn']"/>
                        </xsl:when>
                        <xsl:otherwise/>
    ...
    <xsl:template name="tExternalLinks">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="eac:eac-cpf/eac:cpfDescription/eac:relations/eac:resourceRelation[@xlink:role='archivalRecords']">
                        ...
                        <xsl:if test="$pBirthYr='true'">
                            ...
                            <xsl:choose>
                                <xsl:when
                                    ...
                                    <xsl:if
                                        test="string-length(translate(substring-before(normalize-space(eac:eac-cpf/eac:cpfDescription/eac:identity/eac:nameEntry[2][preceding-sibling::eac:entityType='person']/eac:part),'-'),$vDigits,''))&lt;string-length(substring-before(normalize-space(eac:eac-cpf/eac:cpfDescription/eac:identity/eac:nameEntry[2][preceding-sibling::eac:entityType='person']/eac:part),'-'))">
                                        <xsl:value-of
